...
def dense_search(query: str, k: int = 10) -> list[dict[str, Any]]:
    ...
    vectorstore = load_vectorstore()
    print(f"[timing] dense load_vectorstore call: {time.perf_counter() - t0:.2f}s")

    t0 = time.perf_counter()
    raw_results = vectorstore.similarity_search_with_score(query, k=k)
    print(f"[timing] dense similarity search: {time.perf_counter() - t0:.2f}s")
    ...
    print(f"[timing] dense total: {time.perf_counter() - start_total:.2f}s")
